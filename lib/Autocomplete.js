...
let Autocomplete = React.createClass({
  ...
  propTypes: {
    wrapperComponent: React.PropTypes.oneOfType([
      React.PropTypes.element, // React component classes
      React.PropTypes.func, // Stateless React components
      React.PropTypes.string, // 'div', 'span', etc.
    ]),
  ...
  exposeAPI (el) {
    this.input = el
    NATIVE_API.forEach(ev => this[ev] = (el && el[ev] && el[ev].bind(el)))
  },
  ...
  prepareInputProps (props) {
    ...
    for (let prop in props) {
      if (!(prop in Autocomplete.propTypes)) {
        out[prop] = props[prop]
      }
    }
    return out
  ...
  render () {
    ...
    const { wrapperComponent: Wrapper } = this.props
    return (
      <Wrapper>
        <input
          {...this.prepareInputProps(this.props)}
          role="combobox"
          aria-autocomplete="list"
          autoComplete="off"
          ref={this.exposeAPI}
          onFocus={this.composeEventHandlers(this.handleInputFocus, this.props.onFocus)}
          onBlur={this.composeEventHandlers(this.handleInputBlur, this.props.onBlur)}
          onChange={this.composeEventHandlers(this.handleChange, this.props.onChange)}
          onKeyDown={this.composeEventHandlers(this.handleKeyDown, this.props.onKeyDown)}
          onKeyUp={this.composeEventHandlers(this.handleKeyUp, this.props.onKeyUp)}
          onClick={this.composeEventHandlers(this.handleInputClick, this.props.onClick)}
        />
