...
describe('Autocomplete acceptance tests', () => {
  ...
  it('should allow specifying any event handler via `props`', () => {
    const handlers = ['Focus', 'Blur', 'KeyDown', 'KeyUp', 'Click'];
    const spies = [];
    const props = {};
    handlers.forEach((handler, i) => props[`on${handler}`] = spies[i] = jest.fn());
    const tree = mount(AutocompleteComponentJSX(props));
...
describe('Autocomplete keyPress-><character> event handlers', () => {
  ...
  it('should pass updated `input.value` to `onChange` and replace with `props.value`', done => {
    ...
    let value = '';
    autocompleteWrapper.setProps({ value, onChange(e) { value = e.target.value; } });
